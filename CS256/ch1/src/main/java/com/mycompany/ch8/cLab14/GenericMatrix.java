...
public abstract class GenericMatrix<E extends Object> {
    ...
    protected abstract E multiple(E o1, E o2);
    ...
    public E[][] multiplyMatrix(E[][] matrix1, E[][] matrix2) {
        if (matrix1[0].length != matrix2.length) {
            throw new RuntimeException("The maricies do not have compatible size");
        }
        ...
        for (int i = 0; i < result.length; i++) {
            for (int j = 0; j < result[0].length; i++) {
                ...
                for (int k = 0; k < matrix1[0].length; k++) {
                    result[i][j] = add(result[i][j], multiple(matrix1[i][k], matrix2[k][j]));
                }
            }
        }
        return result;
